...
public class DimensionViewParamTransformerTest extends WMSTestSupport {
    ...
    public void setUp() throws Exception {
        ...
        transformer = applicationContext.getBean("getMapCallback",
                org.geoserver.wms.dimension.viewparam.DimensionSQLViewParamRequestTransformer.class);
        transformer.setResourceNamesToMatch(
                Arrays.<Name> asList(new NameImpl("http://geoserver.org", "layerOne")));
        ...
        transformer.setViewParameterName(DimensionName.ELEVATION, RangeLimitType.START,
                "elevationStart");
        transformer.setViewParameterName(DimensionName.ELEVATION, RangeLimitType.END,
                "elevationEnd");
        ...
        request.setLayers(Arrays.asList(mockData.addFeatureTypeLayer("layerOne", Point.class),
                mockData.addFeatureTypeLayer("layerTwo", Point.class)));
        request.setStyles(Arrays.asList(mockData.getDefaultStyle().getStyle(),
        ...
        final DummyRasterMapProducer producer = new DummyRasterMapProducer() {
             ...
             * 
             * for (Object currentTime : times) { map = executeInternal(mapContent, request, delegate, Arrays.asList(currentTime), elevations);
             * 
             * // remove layers to start over again mapContent.layers().clear();
             * 
             * // collect the layer images.add(((RenderedImageMap)map).getImage()); ^^^^^^^^^^^^^^^^^ }
             */
            @Override
    ...
    public void testMultipleTimeInstancesTransformation() throws Exception {
        ...
        request.setTime(Arrays.<Object> asList((new DateTime(timeStr1).toDate()),
                (new DateTime(timeStr2).toDate())));
        WebMap map = null;

        try {
            map = getMapOp.run(request);
            assertViewParamSet(request, "timeStart",
                    "2004-12-13T23:59:59.999+00:00" + "," + "2004-12-14T00:00:00.000+00:00");
            assertViewParamSet(request, "timeEnd",
                    "2004-12-13T23:59:59.999+00:00" + "," + "2004-12-14T00:00:00.000+00:00");
        } finally {
            if (map != null) {
                map.dispose();
    ...
    public void testSingleTimeRangeTransformation() throws Exception {
        ...
        request.setTime(Arrays.asList((Object) new DateRange((new DateTime(timeBeginStr)).toDate(),
    ...
    public void testMultipleTimeRangesTransformation() throws Exception {
        ...
        request.setTime(Arrays.<Object> asList(
                new DateRange((new DateTime(timeBeginStr1)).toDate(),
                        (new DateTime(timeEndStr1)).toDate()),
                new DateRange((new DateTime(timeBeginStr2)).toDate(),
        ...
        try {
            map = getMapOp.run(request);
            assertViewParamSet(request, "timeStart",
                    "2004-12-13T23:59:59.999+00:00" + "," + "2004-12-31T23:59:59.999+00:00");
            assertViewParamSet(request, "timeEnd",
                    "2004-12-14T00:00:00.000+00:00" + "," + "2005-01-01T00:00:00.000+00:00");
        } finally {
            if (map != null) {
                map.dispose();
            }
    ...
    public void testSingleElevationTransformation() throws Exception {
        ...
        WebMap map = null;
        String elevFormatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevStr));
        try {
            map = getMapOp.run(request);
    ...
    public void testMultipleElevationInstancesTransformation() throws Exception {
        ...
        request.setElevation(Arrays.<Object> asList(new Double(elevStr1), new Double(elevStr2)));
        WebMap map = null;
        String elev1Formatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevStr1));
        String elev2Formatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevStr2));
        try {
            map = getMapOp.run(request);
    ...
    public void testSingleElevationRangeTransformation() throws Exception {
        ...
        request.setElevation(Arrays.<Object> asList(new NumberRange<Double>(Double.class,
                new Double(elevStartStr), new Double(elevEndStr))));
        WebMap map = null;
        String elevStartFormatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevStartStr));
        String elevEndFormatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevEndStr));
        try {
            map = getMapOp.run(request);
    ...
    public void testMultipleElevationRangesTransformation() throws Exception {
        ...
        request.setElevation(Arrays.<Object> asList(
                new NumberRange<Double>(Double.class, new Double(elevStartStr1),
                        new Double(elevEndStr1)),
                new NumberRange<Double>(Double.class, new Double(elevStartStr2),
                        new Double(elevEndStr2))));
        WebMap map = null;
        String elevStart1Formatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevStartStr1));
        String elevStart2Formatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevStartStr2));
        String elevEnd1Formatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevEndStr1));
        String elevEnd2Formatted = String.format(transformer.getElevationFormatPattern(),
                Double.parseDouble(elevEndStr2));
        try {
            map = getMapOp.run(request);
            assertViewParamSet(request, "elevationStart",
                    elevStart1Formatted + "," + elevStart2Formatted);
            assertViewParamSet(request, "elevationEnd",
                    elevEnd1Formatted + "," + elevEnd2Formatted);
        } finally {
            if (map != null) {
                map.dispose();
            }
    ...
    public void testLayerMatching() throws Exception {
        ...
        request.setTime(Arrays.<Object> asList((new DateTime(timeStr).toDate())));
        transformer.setResourceNamesToMatch(
                Arrays.<Name> asList(new NameImpl("http://geoserver.org", "nonExistentLayer")));
        WebMap map = null;
        try {
            map = getMapOp.run(request);
        ...
        transformer.setResourceNamesToMatch(Collections.<Name> emptyList());
        try {
            map = getMapOp.run(request);
            assertViewParamNotSet(request, "timeStart");
            assertViewParamNotSet(request, "timeEnd");
        ...
        transformer.setResourceNamesToMatch(null);
        try {
            map = getMapOp.run(request);
            assertViewParamSet(request, "timeStart", "2004-12-13T23:59:59.999+00:00");
            assertViewParamSet(request, "timeEnd", "2004-12-13T23:59:59.999+00:00");
    ...
    public void testTimeParameterCustomNames() throws Exception {
        ...
        transformer.setViewParameterName(DimensionName.TIME, RangeLimitType.START,
                "someRandomName");
        transformer.setViewParameterName(DimensionName.TIME, RangeLimitType.END,
                "someOtherRandomName");
    ...
    static void assertViewParamSet(GetMapRequest req, String name, String expected) {
        ...
        if (!found) {
            ...
            fail("value '" + expected + "' expected for view param '" + name
                    + "' but instead got these values for some layers: "
                    + nonMatchingValues.toString());
        }
